...
'.$content.'
        ...
        protected function _drawRectangle($x, $y, $w, $h, $border, $padding, $margin, $background)
        {
            ...
            if (is_array($inTL) && count($inTL)>1 && ($inTL[0]<=0 || $inTL[1]<=0)) $inTL = null;
            if (is_array($inTR) && count($inTR)>1 && ($inTR[0]<=0 || $inTR[1]<=0)) $inTR = null;
            if (is_array($inBR) && count($inBR)>1 && ($inBR[0]<=0 || $inBR[1]<=0)) $inBR = null;
            if (is_array($inBL) && count($inBL)>1 && ($inBL[0]<=0 || $inBL[1]<=0)) $inBL = null;
